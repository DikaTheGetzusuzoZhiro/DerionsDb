...
{ regex: /discord(?:app)?.com/api/webhooks/[A-Za-z0-9/-]+/i, desc: "Link Discord Webhook", sev: 5 },
...
4. Info Konteks Real-time: Saat ini adalah hari Sabtu, 11 April 2026, pukul 09:39 WIB. Posisi server di Banjarnegara, Jawa Tengah, Indonesia. Gunakan informasi ini jika pengguna bertanya soal waktu atau lokasi tanpa perlu mengarahkan mereka untuk menggunakan command tertentu.    },     { role: "user", content: input }     ],     temperature: 0.7     },     {     headers: {     Authorization:Bearer ${GROQ_API_KEY}`,
...
.addFields(
{ name: '📡 Ping:', value: \${client ? client.ws.ping : 0}ms` 🟢, inline: true },     { name: '🤖 Core:', value: '🟢 Online', inline: true },     { name: '🧠 AI Engine:', value: '🟢 Aktif`', inline: true }
)
...
.setFooter({ text: 'Created By Tama COMUNITY' })],
components: [
...
if (message.channel.id === scannerChannelId && message.attachments.size > 0) {    
    ...
    try {    
        ...
        const embed = new EmbedBuilder()    
            ...
            .setFooter({ text: "Deteksi by Tama COMUNITY" })    
            .setTimestamp();    
    ...
    if (commandName === 'upload') {    
        try {    
            ...
            const embed = new EmbedBuilder()    
                ...
                .setFooter({ text: `@Tama comunity | ${new Date().toLocaleDateString('id-ID')}` });    
...
if (interaction.isButton()) {    
    if (interaction.customId === 'spam_set_webhook') {    
        ...
        modal.addComponents(    
            ...
            new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('in_webhook_msg').setLabel('Pesan Spam').setStyle(TextInputStyle.Paragraph).setRequired(true).setValue('WEBHOOK INI TELAH DIHANCURKAN OLEH Tama COMUNITY ANTI KEYLOGGER!'))    
        );    
    ...
    if (interaction.customId === 'spam_set_tele') {    
        ...
        modal.addComponents(    
            ...
            new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('in_tele_msg').setLabel('Pesan Spam').setStyle(TextInputStyle.Paragraph).setRequired(true).setValue('BOT INI TELAH DIHANCURKAN OLEH Tama COMUNITY ANTI KEYLOGGER!'))    
        );    
        return interaction.showModal(modal);    
    }    
